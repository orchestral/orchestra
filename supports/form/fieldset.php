		...
		$field = function ($row, $control, $templates = array()) use ($type, $config) 
		{
			...
			switch (true)
			{
				case (in_array($type, array('select', 'input:select'))) :
					...
					$data->method('select')
						->attributes(HTML::compile($control->attributes, $config['select']))
						->options($options);
					break;
				...
				case (in_array($type, array('textarea', 'input:textarea'))):
					$data->method('textarea')
						->attributes(HTML::compile($control->attributes, $config['textarea']));
					break;
				
				case (in_array($type, array('password', 'input:password'))) :
					$data->method('password')
						->attributes(HTML::compile($control->attributes, $config['password']));
					break;
				...
				case (isset($methods[0]) and $methods[0] === 'input') :
					...
					$data->method('input')
						->type($methods[1])
						->attributes(HTML::compile($control->attributes, $config['input']));
					break;
