...
}if(m.isNull(z)){z=k.document;}E=A.split(" ");C="GET";x.put({name:o,type:"GET",query:"",data:"",dataType:!m.isUndefined(y)?y:"json",id:"",object:z});if(E.length===1){D=E[0];
}else{if(m.indexOf(["POST","GET","PUT","DELETE"],E[0])>-1){C=E[0];}D=E[1];if(C!=="GET"){B=D.split("?");if(B.length>1){D=B[0];x.put("query",B[1]);}}D=D.replace(":baseUrl",w.get("baseUrl",""));
x.put({type:C,uri:D});}x.put("id","#"+h(x.get("object")).attr("id"));return this;},execute:function s(){var y,z;y=this;z=h(y.get("object")).serialize()+"&"+y.get("query");
if(z==="?&"){z="";}y.executed=true;j.fire("Request.beforeSend",[y]);j.fire("Request.beforeSend: "+o,[y]);y.config.beforeSend(y);h.ajax({type:y.get("type"),dataType:y.get("dataType"),url:y.get("uri"),data:z,complete:function x(C){var B,A;
if(C.responseText!==""){B=e(C.responseText);A=C.status;if(!m.isUndefined(B)&&B.hasOwnProperty("errors")){j.fire("Request.onError",[B.errors,A,y]);j.fire("Request.onError: "+o,[B.errors,A,y]);
